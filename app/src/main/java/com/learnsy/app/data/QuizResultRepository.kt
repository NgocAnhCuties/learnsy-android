...
class QuizResultRepository {
    ...
    suspend fun saveResult(
        ...
        android.util.Log.i(
            "LearnsySaveResult",
            "Lưu kết quả — SUPA_URL: ${com.learnsy.app.BuildConfig.SUPA_URL}, studentId=$studentId, lessonId=$lessonId"
        )
        ...
        try {
            ...
            android.util.Log.e(
                "LearnsySaveResult",
                "Upsert quiz_results thất bại — SUPA_URL đang dùng: ${com.learnsy.app.BuildConfig.SUPA_URL}",
                e
            )
            throw e
        }
    ...
    suspend fun fetchHistory(studentId: String): Pair<List<HistoryEntry>, Map<String, List<PerQuestionResult>>> {
        ...
        return try {
            ...
            rows.forEach { row ->
                val scoreDouble = row.score.toDoubleOrNull() ?: return@forEach
                val millis = try {
                ...
                row.per_q?.let { raw ->
                    try {
                        val decoded = json.decodeFromString<List<PerQRow>>(raw)
